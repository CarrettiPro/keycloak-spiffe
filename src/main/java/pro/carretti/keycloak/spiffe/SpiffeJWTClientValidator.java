...
// Stolen from JWTClientValidator

public class SpiffeJWTClientValidator {

    private static final Logger logger = Logger.getLogger(SpiffeJWTClientValidator.class);
    ...
    public SpiffeJWTClientValidator(ClientAuthenticationFlowContext context, String clientAuthenticatorProviderId) {
    ...
    public boolean validateClient() {
        ...
        // In JWT-SVIDs, sub and iss are different
...
//        }
    ...
    public void validateSPIFFE(String trustDomain, String oidcIssuer) {
        if (token == null) throw new IllegalStateException("Incorrect usage. Variable 'token' is null. Need to read token first before validateToken");

        try {
            URI uri = URI.create(token.getSubject());
            if (!uri.getScheme().equals("spiffe")) {
                throw new RuntimeException("subject is not a SPIFFE ID");
            }

            if (!trustDomain.equals(uri.getAuthority())) {
                throw new RuntimeException("SPIFFE trust domain mismatch");
            }

            String issuer = token.getIssuer();
            if (issuer == null) {
                throw new RuntimeException("missing issuer");
            } else if (!oidcIssuer.equals(issuer)) {
